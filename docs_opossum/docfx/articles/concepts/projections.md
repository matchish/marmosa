<!-- source: docs/specifications/dcb-projections.md — keep in sync -->
...
Depending on your use case, one view may serve better than the other â€” or you might need both. That's where projections come in: they translate an event-based history into a state-based model tailored to specific needs.
...
- **`initialState`** â€“ the starting value before any events are applied
- **`handlers`** â€“ a map of event type names to reducer functions `(state, event) => state`
...
In the context of DCB, projections are typically used to reconstruct the minimal model required to validate constraints â€” usually in response to a command. It is therefore paramount to minimize the time needed to rebuild the Decision Model by loading only the events that are relevant to validating the received command.
...
Decision Models are usually only concerned with single entities. For example, to determine whether a course with a specific id exists, only `CourseDefined` events tagged for **that course** should be loaded â€” not all `CourseDefined` events globally.
...
- **`initialState`** â€“ the starting state value
- **`apply(state, event)`** â€“ applies a single event to the current state
- **`query`** â€“ a `Query` value that can be used to filter events (manually or translated to an Event Store query)
...
- It makes the projection more "greedy" â€” if only part of the state is needed for a given decision, it still loads more events than necessary, widening the consistency boundary needlessly.
...
The state of the composite projection is an object keyed by each sub-projection name. The resulting query matches only events that are relevant to **at least one** of the composed projections â€” no more, no less.
...
- **`state`** â€“ contains the composed state of all projections
- **`appendCondition`** â€“ can be passed to the Event Store's `append()` method to enforce consistency (see [DCB Specification](dcb.md) for details)

The `appendCondition` ties the decision model to a specific position in the event stream. If any relevant event is appended by a concurrent operation between reading and writing, the append will be rejected â€” enforcing the **Dynamic Consistency Boundary**.
...
| **`handlers`** | Map of event type â†’ reducer; also defines which event types to load |
...
Projections play a fundamental role in DCB and Event Sourcing as a whole. The ability to combine multiple simple projections into more complex ones â€” tailored to specific use cases â€” unlocks a range of possibilities that can influence application design. Keeping projections small and single-purpose is key: it keeps the consistency boundary as narrow as possible and the code easy to reason about.
