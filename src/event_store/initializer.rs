use crate::event_store::OpossumOptions;
...
mod tests {
    ...
    async fn initialize_with_relative_path_creates_directories() {
        ...
        assert!(
            storage
                .read_dir("./test-data/CourseManagement")
                .await
                .is_ok()
        );
    }
    ...
    async fn initialize_with_nested_path_creates_all_directories() {
        let storage = alloc::sync::Arc::new(InMemoryStorage::new());
        let options =
            OpossumOptions::new("/test_root/level1/level2/level3").use_store("CourseManagement");
        let initializer = StorageInitializer::new(storage.clone(), options);
        ...
        assert!(
            storage
                .read_dir("/test_root/level1/level2/level3")
                .await
                .is_ok()
        );
        assert!(
            storage
                .read_dir("/test_root/level1/level2/level3/CourseManagement")
                .await
                .is_ok()
        );
    }
