...
mod tests {
    ...
    fn command_result_generic_constructor_sets_properties_correctly() {
        let result = CommandResultWith::new(
            true,
    ...
    fn command_result_typical_usage_pattern_works_as_expected() {
        ...
        assert_eq!(
            ...
            Some(String::from("Command execution failed"))
        );
    }
    ...
    fn command_result_generic_typical_usage_pattern_works_as_expected() {
        fn execute_query(should_succeed: bool) -> CommandResultWith<alloc::vec::Vec<String>> {
            if should_succeed {
                CommandResultWith::ok(alloc::vec![
                    String::from("Result1"),
                    String::from("Result2")
                ])
            } else {
        ...
        assert_eq!(
            fail_result.error_message,
            Some(String::from("Query execution failed"))
        );
    }
    ...
    fn test_event_data_deserialization_no_alloc() {
        ...
        let parsed: EventData = from_slice(json).unwrap();
    ...
    fn deserialize_with_valid_json_returns_record() {
        let evt = make_event("TestEvent", &[]);
        let json_bytes = serde_json::to_vec(&evt).unwrap();
        let deserialized: EventRecord = serde_json::from_slice(&json_bytes).unwrap();
    ...
    fn deserialize_with_invalid_json_fails() {
        ...
        let res: Result<(EventRecord, usize), _> = serde_json::from_slice(invalid_json);
        assert!(res.is_err());
    }
