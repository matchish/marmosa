...
use marmosa::projections::{
    NoopProjectionTagProvider, ProjectionStore, ProjectionTagProvider,
...
async fn get_store()
-> StorageBackendProjectionStore<Arc<InMemoryStorage>, TestProjection, TestProjectionTagProvider> {
    let _clock = common::FakeClock::new(0);
    let storage = Arc::new(InMemoryStorage::new());
    StorageBackendProjectionStore::new_with_tag_provider(
        storage,
        "TestProjection".to_string(),
        TestProjectionTagProvider,
    )
}
...
async fn query_by_tag_async_returns_projections_matching_tag_async() {
    ...
    let proj1 = TestProjection {
        id: "1".to_string(),
        status: "Active".to_string(),
        tier: "Premium".to_string(),
    };
    let proj2 = TestProjection {
        id: "2".to_string(),
        status: "Active".to_string(),
        tier: "Basic".to_string(),
    };
    let proj3 = TestProjection {
        id: "3".to_string(),
...
async fn query_by_tags_async_and_logic_is_correct_when_first_tag_index_is_larger_than_second_async()
{
    ...
    let proj4 = TestProjection {
        id: "4".to_string(),
        status: "Inactive".to_string(),
        tier: "Professional".to_string(),
    };
    let proj5 = TestProjection {
        id: "5".to_string(),
        status: "Inactive".to_string(),
        tier: "Professional".to_string(),
    };
...
async fn query_by_tag_async_without_tag_provider_returns_empty_async() {
    let _clock = common::FakeClock::new(0);
    let storage = Arc::new(InMemoryStorage::new());
    let store: StorageBackendProjectionStore<
        Arc<InMemoryStorage>,
        TestProjection,
