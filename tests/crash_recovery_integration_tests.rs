...
async fn append_after_crash_preserves_orphaned_events_and_allocates_next_positions() {
    ...
    let pre_event1 = EventData {
        ...
        event: DomainEvent {
            ...
            data: serde_json::to_string(&OrphanedEvent {
                data: "pre-crash-1".to_string(),
            })
            .unwrap(),
            tags: vec![],
        },
    ...
    store
        .append_async(vec![pre_event1, pre_event2], None)
        .await
        .unwrap();

    // Verify: ledger at position 2
    let events_before_crash = store
    ...
    let orphaned_sequenced = EventRecord {
        ...
        event: DomainEvent {
            event_type: "OrphanedEvent".to_string(),
            data: serde_json::to_string(&OrphanedEvent {
                data: "orphaned-crash-event".to_string(),
            })
            .unwrap(),
            tags: vec![],
        },
    ...
    let post_event = EventData {
        ...
        event: DomainEvent {
            ...
            data: serde_json::to_string(&PostRecoveryEvent {
                data: "post-recovery".to_string(),
            })
            .unwrap(),
            tags: vec![],
        },
    ...
    let orphan_read: OrphanedEvent = serde_json::from_str(
        all_events[2]
            .event
            .data
            .replace("\\\"", "\"")
    ...
    let post_read: PostRecoveryEvent = serde_json::from_str(
        all_events[3]
            .event
            .data
            .replace("\\\"", "\"")
...
async fn read_after_crash_recovery_returns_all_events_in_correct_order() {
    ...
    for i in 0..3 {
        let data: OrphanedEvent = serde_json::from_str(
            all_events[i]
                .event
                .data
                .replace("\\\"", "\"")
    ...
    for i in 5..7 {
        let data: PostRecoveryEvent = serde_json::from_str(
            all_events[i]
                .event
                .data
                .replace("\\\"", "\"")
