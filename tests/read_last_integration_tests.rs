...
async fn create_next_invoice_async(store: Arc<OpossumStore<Arc<InMemoryStorage>, FakeClock>>) -> i32 {
...
async fn read_last_async_after_first_invoice_returns_it_async() {
    ...
    store.append_single_async(event.to_domain_event().build("inv-1"), None).await.unwrap();
...
async fn read_last_async_after_multiple_invoices_returns_highest_position_async() {
    ...
    for i in 1..=3 {
        ...
        store.append_single_async(event.to_domain_event().build(&format!("inv-{}", i)), None).await.unwrap();
    }
...
async fn read_last_async_with_interleaved_event_types_returns_only_last_match_async() {
    ...
    store.append_single_async(event1.to_domain_event().build("inv-1"), None).await.unwrap();
    
    // Append something else to interleave
    #[derive(Serialize)]
    struct OtherEvent { val: i32 }
    let other = OtherEvent { val: 42 };
    store.append_single_async(other.to_domain_event().build("other-1"), None).await.unwrap();
    
    // Append invoice 2
    let event2 = InvoiceCreatedEvent { invoice_number: 2 };
    store.append_single_async(event2.to_domain_event().build("inv-2"), None).await.unwrap();
...
async fn invoice_numbering_sequential_creation_forms_continuous_sequence_async() {
    ...
    let mut numbers: Vec<i32> = events.into_iter()
        .map(|e| {
...
async fn invoice_numbering_append_condition_rejects_stale_decision_async() {
    ...
    let err = store.append_single_async(event2.to_domain_event().build("inv-duplicate"), Some(condition)).await;
