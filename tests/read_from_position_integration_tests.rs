...
fn create_store() -> (Arc<InMemoryStorage>, OpossumStore<Arc<InMemoryStorage>, FakeClock>) {
...
async fn append_shipment_async(store: &impl EventStoreExt, shipment_id: &str) {
    let event = EventData {
        ...
        event: DomainEvent {
            ...
            tags: vec![Tag { key: "shipmentId".to_string(), value: shipment_id.to_string() }],
        },
        metadata: None,
    };
    store.append_single_async(event, None).await.unwrap();
...
async fn read_async_with_from_position_and_tag_filter_returns_only_matching_tags_after_position() {
    let (_, store) = create_store();
    let tag = Tag { key: "orderId".to_string(), value: "o-special".to_string() };
